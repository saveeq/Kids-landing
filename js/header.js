...
function blackColor(navSelector = ".nav-list-item-link") {
  ...
  const headbtnMobile = document.querySelector(".feedbackButton-header-mobile");
...
function changeImageOnScroll(
  imgSelector = ".logo__dentservice",
  imgSelectorMobile = ".logo__dentservice-mobile", // селектор изображения
  ...
  const imgElementMobile = document.querySelector(imgSelectorMobile);
  const originalSrc = imgElement.src; 
  const originalSrcMobile = imgElementMobile.src;
  ...
  window.addEventListener("scroll", () => {
    ...
    if (scrollPositionVH > scrollThreshold) {
      ...
      imgElementMobile.src = newImageSrc;
    } else {
      imgElement.src = originalSrc; // возвращаем исходное
      imgElementMobile.src = originalSrcMobile;
    }
  });
}
